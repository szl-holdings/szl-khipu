...
from .causal_lm_reference import CausalLMWeights
...
class CausalLMTensorNames:
    ...
    def validate(self) -> None:
        ...
        if any(not isinstance(value, str) or not value or len(value) > 1024 for value in values):
            raise CausalLMMappingError("global tensor names must be non-empty bounded strings")
        if self.embedding == self.final_norm:
            raise CausalLMMappingError("embedding and final norm names must differ")
...
def _map_layer(
    ...
    *,
    ...
    requests = (
        ...
        ("gate_proj", names.gate_proj, (intermediate, hidden), "transpose_2d"),
        ("up_proj", names.up_proj, (intermediate, hidden), "transpose_2d"),
        ("down_proj", names.down_proj, (hidden, intermediate), "transpose_2d"),
    )
...
def map_causal_lm(
    ...
    *,
    ...
    if spec.attention_bias or spec.mlp_bias:
        raise CausalLMMappingError("bias-bearing model mapping is not implemented")
