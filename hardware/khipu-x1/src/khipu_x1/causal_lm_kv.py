...
This module provides a correctness-oriented complete-model cache path built on
the Wave 6 decoder block. Cache state is returned as immutable snapshots so a
failed or later operation does not mutate an earlier state object. Execution is
NumPy/CPU only and carries no accelerator or performance claim.
"""
...
def _clone_chain(chain: ReceiptChain | None) -> ReceiptChain:
    if chain is None:
        return ReceiptChain()
    chain.require_valid()
    cloned = copy.deepcopy(chain)
    cloned.require_valid()
    return cloned
        ...
        *,
        ...
        if attention_mode == "yarqa":
            ...
            if canals.flags.writeable or np.any(canals < 0):
                raise CausalLMKVError("YARQA KV canal snapshot must be read-only and non-negative")
...
class CausalLMKVState:
    ...
    def validate(self, weights: CausalLMWeights, spec: TransformerSpec) -> None:
        ...
        expected_manifest = weights.manifest_digest()
        if self.weights_manifest_digest != expected_manifest:
        ...
        expected_digest = hashlib.sha256(canonical_json_bytes(self.payload())).hexdigest()
...
def _build_state(
    *,
    ...
    provisional = CausalLMKVState(
        layers=snapshots,
        batch_size=batch_size,
        sequence_length=sequence_length,
        max_sequence=spec.max_position_embeddings,
        attention_mode=attention_mode,
        weights_manifest_digest=weights_manifest_digest,
...
def prefill_causal_lm(
    ...
    *,
    ...
    config = _config(spec, attention_mode)
    ...
    normalized, logits = _logits(hidden, weights, spec)
    ...
    receipt_chain.append(
        "causal_lm_kv_prefill_completed",
        {
            ...
            "weights_manifest_digest": manifest_digest,
            "source_config_digest": spec.source_config_digest,
            "kv_state_digest": state.state_digest,
    ...
    state = _build_state(
        ...
        weights_manifest_digest=state.weights_manifest_digest,
        chain=receipt_chain,
    )
    state.validate(weights, spec)
    return KVForwardResult(
        token_ids=ids,
        hidden=normalized,
        logits=logits,
        state=state,
...
def decode_causal_lm_step(
    ...
    *,
    ...
    config = _config(spec, state.attention_mode)
    snapshots: list[LayerKVSnapshot] = []
    for layer_index, layer_weights in enumerate(weights.layers):
        cache = state.layers[layer_index].to_cache(max_sequence=state.max_sequence)
        result = run_decoder_block(
            hidden,
            layer_weights,
            config,
    ...
    normalized, logits = _logits(hidden, weights, spec)
    new_length = state.sequence_length + 1
    ...
    receipt_chain.append(
        "causal_lm_kv_decode_step_completed",
        {
            ...
            "kv_state_digest": next_state.state_digest,
            "cache_update": "FUNCTIONAL_SNAPSHOT",
            "hardware_status": "UNAVAILABLE",
            "energy_j": None,
            "energy_status": "UNAVAILABLE",
    ...
    receipt_chain.require_valid()
    next_state = _build_state(
        snapshots=next_state.layers,
        batch_size=next_state.batch_size,
        sequence_length=next_state.sequence_length,
        spec=spec,
        attention_mode=next_state.attention_mode,
        weights_manifest_digest=next_state.weights_manifest_digest,
        chain=receipt_chain,
    )
...
def greedy_generate_cached(
    ...
    *,
    ...
    if attention_mode == "yarqa":
        if generated_canal_id is None:
            raise CausalLMKVError("YARQA generation requires generated_canal_id")
        if (
            not isinstance(generated_canal_id, int)
            or isinstance(generated_canal_id, bool)
            or generated_canal_id < 0
        ):
            raise CausalLMKVError("generated_canal_id must be non-negative")
