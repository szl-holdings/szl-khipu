...
def test_state_digest_weight_binding_and_step_contracts_fail_closed() -> None:
    spec = _spec(layers=1)
    weights = _weights(spec)
    prefill = prefill_causal_lm(np.array([[1, 2]], dtype=np.int64), weights, spec)

    tampered_state = replace(prefill.state, state_digest="0" * 64)
    ...
    with pytest.raises(CausalLMKVError, match="canal_id"):
        decode_causal_lm_step(
            np.array([[3]], dtype=np.int64),
            replace(prefill.state, attention_mode="yarqa"),
            weights,
            spec,
        )


def test_capacity_and_generation_budget_fail_closed() -> None:
    spec = _spec(layers=1)
    weights = _weights(spec)
    prompt = np.ones((1, spec.max_position_embeddings), dtype=np.int64)
    state = prefill_causal_lm(prompt, weights, spec).state
