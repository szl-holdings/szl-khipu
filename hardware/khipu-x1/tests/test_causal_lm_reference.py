...
def test_yarqa_complete_model_executes_with_explicit_canals(tmp_path: Path) -> None:
    spec = _spec(tied=True, layers=1)
    _write_safetensors(tmp_path / "model.safetensors", _arrays(spec))
    inventory = inventory_local_model(tmp_path, hash_files=True, hash_tensors=True)
...
def test_global_mapping_and_resource_bounds_fail_closed(tmp_path: Path) -> None:
    ...
    duplicate_names = CausalLMTensorNames(
        embedding="model.embed_tokens.weight",
        final_norm="model.norm.weight",
        lm_head="model.embed_tokens.weight",
    )
    with pytest.raises(CausalLMMappingError, match="must be unique"):
        map_causal_lm(other, inventory, spec_two, names=duplicate_names)
